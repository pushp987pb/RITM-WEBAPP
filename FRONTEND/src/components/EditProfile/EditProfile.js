...
function EditProfile(){
    ...
    async function updateUserDetails(userObj) {
        try {
          ...
          isTemple ? dispatch(updateTemple(updatedObj)).then((updatedTemple)=>{
            if(updatedTemple){
              navigate("/temple-profile")
            }
          }) : dispatch( updateUser(updatedObj) ).then( (updatedUser)=>{
            if(updatedUser){
            ...
            isTemple ? (
              <form name='form1' onSubmit={handleSubmit(updateUserDetails)}>
                ...
                <div className="field">
                  ...
                  <input {...register("mobileNumber")} type="tel" required/>
                </div>
                <div className="field">
